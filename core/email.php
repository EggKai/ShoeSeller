...
function sendReceiptEmail($order, $orderItems) {
    ...
    try {
        ...
        $mail->addAddress($order['email'], explode('@',$order['email'])[0]);  // Using order email and name
        
        // Set email format to HTML
        $mail->isHTML(true);
        $mail->Subject = "Order Receipt: Order #{$order['id']}";
        
        // Build the HTML receipt (similar to your reciept.php layout)
        $html = '<div style="font-family: Lucida Sans, Lucida Sans Regular, Lucida Grande, Lucida Sans Unicode, sans-serif; max-width:600px; margin:0 auto; padding: 20px; border: 1px solid #ccc;">';
        ...
        $html .= '<div style="text-align: center; margin-top: 20px; border-radius: 2rem;">';
        ...
        $mail->Body    = $html;
        $mail->AltBody = "Order Receipt\nOrder Number: {$order['id']}\nDate: {$order['created_at']}\nEmail: {$order['email']}\nTotal: $" . number_format($order['total_price'], 2);
        
        $mail->send();
        return true;
    } catch (Exception $e) {
        // Log error or handle it as necessary.
        echo "Receipt email could not be sent. Mailer Error: {$mail->ErrorInfo}";
        logError("Receipt email could not be sent. Mailer Error: {$mail->ErrorInfo}");
        return false;
    }
}
